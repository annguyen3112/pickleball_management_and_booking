...
class AdminController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        //
        $orderRecentUpdated = Order::query()->orderByDesc('updated_at')->limit(10)->get();
        //top customer month
    ...
    {
        $title = 'Equipment';
        $equipments = Equipment::query()->orderByDesc('id')->get();
    ...
    public function checkout(string $id)
    {
        ...
        $isCheckout = ($order->status == OrderStatusEnum::Paid || $order->status == OrderStatusEnum::Cancel);

        $arr = [];
        if ($order->status == OrderStatusEnum::Paid) {
            $arr['status'] = 'success';
            $arr['message'] = 'Đã thanh toán';
        } elseif ($order->status == OrderStatusEnum::Cancel) {
            $arr['status'] = 'danger';
            $arr['message'] = 'Đã hủy';
        } else {
            $arr['status'] = 'warning';
    ...
    {
        $title = 'Customer';
        $customers = User::where('role', UserRole::Client)->get();
    ...
    {
        $title = 'Owner';
        $owner = User::where('role', UserRole::CourtOwner)->get();
