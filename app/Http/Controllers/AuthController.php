...
class AuthController extends Controller
{
    ...
    public function systemAdminLogin()
    {
        $title = 'Login';
    ...
    public function processSystemAdminLogin(Request $request)
    {
        $remember = false;
        if($request->has('remember')){
            $remember = true;
        ...
        if(Auth::attempt($credentials, $remember)){
            ...
            return to_route('systemAdmin.dashboard');
        }
    ...
    public function processSystemAdminLogout(Request $request)
    {
        if(Auth::check()){
            Auth::logout();
            $request->session()->invalidate();
            $request->session()->regenerateToken();
            return to_route('systemAdmin.login');
        }
    }
