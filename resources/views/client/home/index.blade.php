                        ...
                        <div
                            ...
                            <div class="portfolio-info">
                                @if ($item->is_maintenance)
                                    <h4 class="text-danger">{{ $item->name }} - {{ $item->pitchType->description }} 
                                    </h4>
                                    <p class="text-danger">{{ $item->pricePerHour() }} - {{ $item->pricePerPeakHour() }}</p>
                                    <p class="text-danger">Bảo trì</p>
                                @else
                                    <h4>{{ $item->name }} - {{ $item->pitchType->description }} </h4>
                                ...
                                @if ($item->images->count() > 0)
                                    <a href="{{ config('app.image') . $item->images[0]->image }}"
                                        ...
                                        title="{{ $item->name }}" data-title="{{ $item->name }}">
                                        <i class="bi bi-eye-fill"></i>
                                    </a>
                                @else
                                    <a href="{{ config('app.image') . 'storage/images/football_pitches/default_football_pitch.png' }}"
                                        data-gallery="portfolioGallery" class="portfolio-lightbox preview-link"
                                        title="{{ $item->name }}" data-title="{{ $item->name }}">
                                        <i class="bi bi-eye-fill"></i>
                                    </a>
